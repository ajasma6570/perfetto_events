...
export default function BlogPage() {
  ...
  return (
    <div className="pt-24 space-y-20">
      ...
      <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3 w-full max-w-[102rem] mx-auto mb-20">
        {blogs.map((blog) => (
          <div
            key={blog.id}
          ...
          >
            <div className="relative h-[298px] w-[460px] overflow-hidden rounded-xl">
              <Image
                src={`/images/blog/${blog.src}`}
                alt={blog.title}
                fill
                className="object-cover h-[298px] w-full transform transition-all duration-500 group-hover:scale-105"
              />
            ...
            <h3 className="text-[28px] leading-tight font-medium mt-2 text-[#001A2E]">
              {blog.title}
            </h3>

            <div className="mt-auto flex flex-col items-start space-y-2 pt-2">
              <p className="text-[22px] text-[#4E4E4E]">Events | {blog.date}</p>
              <button
                onClick={() => handleBlogClick(blog.id)}
                className="border border-[#F7931E] cursor-pointer text-[#001A2E] px-5 lg:px-10 py-3 rounded-full group-hover:text-white group-hover:bg-[#c4161c] group-hover:border-[#c4161c] transform transition-all duration-300"
              >
                Read More
              </button>
            </div>
